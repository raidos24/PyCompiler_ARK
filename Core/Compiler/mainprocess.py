...
def start_compilation_process(self, file):
    ...
    Cette fonction utilise une approche robuste basée sur subprocess.Popen
    inspirée du module "only mod" pour une meilleure gestion des processus:
    - Utilisation de subprocess.Popen au lieu de QProcess
    - Lecture temps réel avec select pour stdout/stderr
    - Meilleure gestion de l'annulation et des sorties restantes
    ...
    # ========================================================================
    # Utiliser l'instance UI du moteur si disponible, sinon créer une nouvelle
    try:
    ...
    # ========================================================================
    # Étape 10: Création d'un objet processus personnalisé
    # ========================================================================
    # Au lieu d'utiliser QProcess, on crée un objet personnalisé pour subprocess
    class SubprocessWrapper:
        def __init__(self, program, args, env, cwd, cancel_file, engine_id, file_path, file_basename):
            ...
            self._grace_kill_timer = None
            self._cancel_requested = False
            self._final_stderr = ""

        def start(self):
            """Démarre le processus subprocess."""
            try:
        ...
        def wait_for_finished(self, timeout_ms):
        ...
        def read_output(self):
            """Lit la sortie en temps réel avec select (inspiré du only mod)."""
            if not self._process:
                return

            try:
                while True:
                    # Vérifier l'annulation coopérative
                    ...
                    for stream in ready:
                        if stream == self._process.stdout:
                            ...
                            if line:
                                # Émettre le signal stdout (simulé)
                                self._handle_stdout(line.rstrip())
                        elif stream == self._process.stderr:
                            line = self._process.stderr.readline()
                            if line:
                                # Émettre le signal stderr (simulé)
                                self._handle_stderr(line.rstrip())
                ...
                if remaining_stderr:
                    self._final_stderr = remaining_stderr
                    for line in remaining_stderr.strip().split("\n"):
                        if line:
                            self._handle_stderr(line.rstrip())

            except Exception as e:
                self._handle_stderr(f"Error reading output: {str(e)}")

        def _handle_stdout(self, data):
            """Gestionnaire pour la sortie standard (simule handle_stdout)."""
            # Appeler handle_stdout avec un objet compatible
            self._emit_output(data, is_stderr=False)

        def _handle_stderr(self, data):
            """Gestionnaire pour la sortie d'erreur (simule handle_stderr)."""
            # Appeler handle_stderr avec un objet compatible
            self._emit_output(data, is_stderr=True)

        def _emit_output(self, data, is_stderr=False):
            """Émet les données de sortie via les gestionnaires existants."""
            # Créer un objet temporaire compatible avec handle_stdout/handle_stderr
            temp_process = type('TempProcess', (), {
            ...
            if is_stderr:
                self._parent.handle_stderr(temp_process, data)
            else:
                self._parent.handle_stdout(temp_process, data)

        def set_parent(self, parent):
            """Définit le parent pour accéder aux méthodes de gestion."""
            self._parent = parent
    ...
    # Créer l'objet processus personnalisé
    process = SubprocessWrapper(
        program=program,
        args=args,
        env=execution_env,
    ...
    process.set_parent(self)

    # ========================================================================
    # Étape 11: Ajout aux processus actifs
    # ========================================================================
    self.processes.append(process)
    self.current_compiling.add(file)

    # ========================================================================
    # Étape 12: Mise à jour optionnelle de l'interface
    # ========================================================================
    ...
    # ========================================================================
    # Étape 13: Configuration du timeout avec arrêt propre puis kill
    # ========================================================================
    try:
        ...
        if timeout_sec and timeout_sec > 0:
            ...
            def _on_timeout(proc=process, seconds=timeout_sec):
                try:
                    self.log.append(
                        f"⏱️ Timeout ({seconds}s) pour {proc.file_basename}. Arrêt en cours…"
                    )
                except Exception:
                    pass
                ...
                def _force_kill(p=proc):
                    if p.is_running():
                        try:
                            self.log.append(
                                f"🛑 Arrêt forcé du processus {p.file_basename} après délai de grâce."
                            )
                        except Exception:
                            pass
                ...
                process._grace_kill_timer = grace
    ...
    # ========================================================================
    # Étape 14: Lancement du processus dans un thread séparé
    # ========================================================================
    def _run_process_in_thread(proc=process):
        """Exécute le processus dans un thread séparé."""
        try:
            proc.start()
            proc.read_output()

            # Appeler handle_finished quand terminé
            return_code = proc.get_return_code()
            exit_status = QProcess.NormalExit if return_code == 0 else QProcess.CrashExit
            self.handle_finished(proc, return_code, exit_status)

        except Exception as e:
            self.log.append(f"❌ Erreur lors de l'exécution du processus: {e}")
            self.handle_finished(proc, -1, QProcess.CrashExit)

    # Lancer dans un thread pour ne pas bloquer l'UI
    process_thread = threading.Thread(target=_run_process_in_thread, daemon=True)
